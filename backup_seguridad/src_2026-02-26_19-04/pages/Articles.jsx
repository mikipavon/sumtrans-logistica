...
import { Search, Plus, FileText, Trash2, Edit, Euro, Tag, Map, Upload, Download } from 'lucide-react';
import { read, utils } from 'xlsx';

export default function Articles({ articles, onAddArticle, onUpdateArticle, onDeleteArticle, tariffs, onAddTariff, onUpdateTariff, onDeleteTariff, onImportArticles, onImportTariffs }) {
        ...
        try {
            ...
            if (importType === 'articles') {
                ...
                const newArticles = jsonData.map(row => {
                    ...
                    return {
                        name: row[findKey(['name', 'nombre', 'articulo', 'servicio'])] || 'Nuevo Servicio',
                        description: row[findKey(['description', 'descripción', 'descripcion', 'detalles'])] || '',
                        price: row[findKey(['price', 'precio', 'coste', 'valor'])] || '0',
                        unit: row[findKey(['unit', 'unidad', 'tipo'])] || 'Unidad'
                    };
                }).filter(a => a.name && a.price); // Basic validation
    ...
    const handleDownloadTemplate = () => {
        const type = activeTab; // 'articles' or 'tariffs'
        const headers = type === 'articles'
            ? [{ Nombre: 'Ejemplo Servicio', Descripcion: 'Descripción del servicio', Precio: '10.50', Unidad: 'Unidad' }]
        ...
        // We need writeFile from xlsx, but we only imported read/utils to save bundle size? 
        // Actually writeFile needs 'fs' in node, but works in browser if full bundle.
        // Let's use simple mechanic or just alert for now if writeFile not avail, 
        // but usually 'xlsx' package has writeFile. Let's try importing it.
        // Re-importing at top level.
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                    <button
                        onClick={() => handleImportClick(activeTab)}
                        className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl font-medium flex items-center gap-2 shadow-lg shadow-emerald-500/20 transition-all hover:scale-105 active:scale-95"
                    >
                        <Upload size={20} />
                        Importar Excel
                    </button>
