...
export default function Dashboard() {
    const stats = [
        { title: 'En Reparto', value: '18', icon: Truck, color: 'bg-blue-500', trend: '+5%' },
        { title: 'Entregados', value: '24', icon: Package, color: 'bg-green-500', trend: '+12%' },
                    ...
                    <div className="space-y-4">
                        {[1, 2, 3].map((i) => (
                            <div key={i} className="flex items-center gap-4 p-3 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-100">
                                <div className={`w-2 h-2 rounded-full ${i === 1 ? 'bg-blue-500' : i === 2 ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                                ...
                                <span className={`text-xs font-semibold px-2 py-1 rounded ${i === 1 ? 'text-blue-600 bg-blue-50' :
                                        i === 2 ? 'text-emerald-600 bg-emerald-50' :
                                            'text-red-600 bg-red-50'
                                    }`}>
                                    {i === 1 ? 'En reparto' : i === 2 ? 'Entregado' : 'Incidencia'}
                                </span>
                            </div>
                        ))}
                    </div>
