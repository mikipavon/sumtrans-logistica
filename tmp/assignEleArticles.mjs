...
async function run() {
    ...
    const eleIds = articlesRows
        .filter(a => String(a.data?.category || '').toUpperCase().includes('ELE'))
        .map(a => a.id);
        
    console.log('Found', eleIds.length, 'articles in the ELE family.');
    ...
    for (const c of targetClients) {
        ...
        if (changed) {
            ...
            updated++;
             ...
             console.log('Client', c.data.name, 'already had all ELE articles.');
        }
    }
