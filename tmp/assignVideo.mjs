...
async function run() {
    const familyQuery = 'VIDE';
    const clientQuery = 'VIDEOCOLOR';
    ...
    // Exact match for category 'VIDE'
    const targetIds = articlesRows
        .filter(a => String(a.data?.category || '').toUpperCase().trim() === familyQuery)
        .map(a => a.id);
    ...
    const targetClients = clients.filter(c => {
        ...
        return name.includes(clientQuery);
    });
