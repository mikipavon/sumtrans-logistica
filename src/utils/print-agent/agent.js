/**
 * agent.js — SUM Logística Print Interceptor Agent
 * 
 * Un servidor TCP ligero y autónomo en Node.js que actúa como Impresora Virtual.
 * ⚡️ SIN DEPENDENCIAS EXTERNAS: Corre directamente en cualquier PC con Node.js instalado.
 * 
 * Funcionalidad:
 * 1. Escucha en puerto TCP 9100 (Estándar de impresión RAW).
 * 2. Aplica filtro inteligente para ignorar nóminas u otros documentos privados.
 * 3. Analiza ZPL y Texto A4 para auto-extraer datos del envío (SSCC, Destinatario, etc.).
 * 4. Envía los datos directamente a la base de datos de Supabase de SUM Logística mediante HTTPS nativo.
 * 5. Redirige opcionalmente el trabajo a la impresora física real de red.
 */

const net = require('net');
const http = require('https'); // Usar https nativo para máxima velocidad y cero npm install
const fs = require('fs');
...
if (fs.existsSync(CONFIG_PATH)) {
  ...
  console.log(`${colors.yellow}⚠ config.json no encontrado. Usando valores por defecto.${colors.reset}`);
}

// Crear carpeta temporal para guardar spools para depuración local
...
// ==========================================
// FILTRO DE PRIVACIDAD E INTELIGENCIA
// ==========================================
...
function runPrivacyFilter(content) {
  ...
  if (matched) {
    return { isSensitive: true, keyword: matched };
  }
  return { isSensitive: false };
}

function detectSSCC(content) {
  // Busca códigos de barras de 18-20 dígitos
...
function parseZpl(content, sscc) {
  // Parsear destinatario en bloque ZPL
...
// ==========================================
// SUBIDA REST DIRECTA A SUPABASE (HTTPS)
// ==========================================
...
function uploadToSupabase(shipment) {
  ...
  // Parsear host de URL de Supabase (ej: mottccbalzdzrgqzfkdl.supabase.co)
  ...
  const options = {
    ...
    headers: {
      ...
      'Prefer': 'resolution=merge-duplicates' // Inserta o machaca si ya existe
    }
  };
...
// ==========================================
// REDIRECCIÓN A IMPRESORA FÍSICA REAL
// ==========================================
...
// ==========================================
// SERVIDOR TCP (IMPRESORA VIRTUAL)
// ==========================================
...
const server = net.createServer((socket) => {
  ...
  socket.on('end', () => {
    ...
    // Guardar archivo spool local para auditoría/debug
    ...
    // 1. Ejecutar Filtro de Privacidad
    ...
    // 2. Comprobar si contiene código logístico SSCC
    ...
    // 3. Procesar y parsear según formato (ZPL térmico o Texto)
    let shipment = null;
    ...
    // 5. Redirigir a impresora física
    forwardPrintJob(rawBuffer);
  });
