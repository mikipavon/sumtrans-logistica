...
try {
    // get App.jsx from main~1 (commit b482409)
    execSync('git show b482409:src/App.jsx > temp_app_old.jsx');
    let oldApp = fs.readFileSync('temp_app_old.jsx', 'utf16le'); 
    // Wait, PowerShell > creates utf-16le!
    // I can read it as utf16le and it will be correct!
    
    // Let's use node to fetch it properly!
    const fileBuf = execSync('git show b482409:src/App.jsx');
    oldApp = fileBuf.toString('utf8');
